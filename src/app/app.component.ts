...
export class AppComponent {
 ...
 total:number=0
 itemQtyMap: Map<number, number> = new Map<number, number>();

insertProduct(e: Product) {
    console.log(e);
    if (!this.cartItems.includes(e)) {
      this.cartItems.push(e);
      this.itemQtyMap.set(e.ISBN, 1); // add new product to quantity map
      this.total = (this.total + e.price.value);
    } else {
      const qty = this.itemQtyMap.get(e.ISBN) || 0;
      this.itemQtyMap.set(e.ISBN, qty + 1); // update existing product quantity
    }
  }
  removeItem(ISBN: number) {
    let itemIndex = this.cartItems.findIndex(obj => obj.ISBN === ISBN);
    let newTotal = (this.total - this.cartItems[itemIndex].price.value * (this.itemQtyMap.get(ISBN) || 0)); // subtract total price of removed quantity 
    this.total = newTotal >= 0 ? newTotal : 0;
    this.itemQtyMap.delete(ISBN); // delete product quantity from map
    this.cartItems.splice(itemIndex, 1);
  }
  
  increaseQnt(item: Product) {
    const qty = this.itemQtyMap.get(item.ISBN) || 0;
    this.itemQtyMap.set(item.ISBN, qty + 1); // increase quantity by 1
    this.total = (this.total + item.price.value);
  }
  
  decreaseQnt(item: Product) {
    const qty = this.itemQtyMap.get(item.ISBN) || 0;
    if (qty > 1) {
      this.itemQtyMap.set(item.ISBN, qty - 1); // decrease quantity by 1 if greater than 1
      this.total = (this.total - item.price.value);
    }
  }
